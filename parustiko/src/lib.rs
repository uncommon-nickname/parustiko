...
use preamble::version_exchange::SshVersionExchange;
...
fn get_header_eoi<T: Read>(arr: &mut T) -> [u8; 51] {
    let mut x = [0; 51];
...
pub fn runner() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client_version = SshVersionExchange::try_build("2.0", "parustiko0.0.1", None)?;

    stream.write_all(client_version.to_string().as_bytes())?;
    std::thread::sleep(Duration::from_millis(10));

    // \r\n -> 13,10
    // from 33 to 51 bytes roughly

    let header = get_header_eoi(&mut stream);
    // let server_version = SshVersionExchange::from_bytes(&mut header);
